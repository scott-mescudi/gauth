...
func TestAddUserSqlite(t *testing.T) {
	...
	var isverified bool
	err = conn.QueryRowContext(t.Context(), "SELECT isverified FROM gauth_user_verification WHERE user_id=?", uuid).Scan(&isverified)
	if err != nil {
		t.Fatalf("error in function: %v", err)
	}
	...
	err = conn.QueryRowContext(t.Context(), "SELECT username, email, role FROM gauth_user WHERE id=?", uuid).Scan(&dbusername, &dbemail, &dbrole)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetUserPasswordAndIDByEmailSqlite(t *testing.T) {
	...
	err = conn.QueryRowContext(t.Context(), "SELECT password_hash FROM gauth_user_auth WHERE user_id=?", uuid).Scan(&storedHash)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSetRefreshTokenSqlite(t *testing.T) {
	...
	var token string
	err = conn.QueryRowContext(t.Context(), "SELECT refresh_token FROM gauth_user_auth WHERE user_id=?", uuid).Scan(&token)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSetUserPassword(t *testing.T) {
	...
	var storedHash string
	err = conn.QueryRowContext(t.Context(), "SELECT password_hash FROM gauth_user_auth WHERE user_id=?", userid).Scan(&storedHash)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDeleteUser(t *testing.T) {
	...
	var exists bool
	err = conn.QueryRowContext(t.Context(), "SELECT EXISTS (SELECT 1 FROM gauth_user WHERE id=?)", userid).Scan(&exists)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSetUserEmail(t *testing.T) {
	...
	err = conn.QueryRowContext(t.Context(), "SELECT email FROM gauth_user WHERE id=?", userid).Scan(&storedEmail)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSetIsverified(t *testing.T) {
	...
	var isVerified bool
	err = conn.QueryRowContext(t.Context(), "SELECT isverified FROM gauth_user_verification WHERE user_id=?", userid).Scan(&isVerified)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSetVerificationTokenAndExpiry(t *testing.T) {
	...
	err = conn.QueryRowContext(t.Context(), "SELECT verification_token, token_expiry FROM gauth_user_verification WHERE user_id=?", userid).Scan(&storedToken, &expiry)
	if err != nil {
		t.Fatal(err)
	}
